...
import { setPosition } from "@/redux/slices/activitySlices";
...
export default function Position() {
  ...
  const cities = useMemo(() =>
    countryIso ? City.getCitiesOfCountry(countryIso)?.map((c) => ({ name: c.name, country: c.countryCode, region: c.stateCode || "City", flag: "📍" })) || [] : [], [countryIso]);
  ...
  const handleNext = () => {
    ...
    dispatch(setPosition(isInternational ? city : OPTIONS.find((o) => o.id === selected)!.label));
  ...
  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: bg }}>
      <KeyboardAwareScrollView contentContainerStyle={{ flexGrow: 1, paddingBottom: 120 }} extraScrollHeight={100} enableOnAndroid keyboardShouldPersistTaps="handled">
        <HeaderQu linkPrv="/(screens)/(activity)/Duration" linkNext="/(screens)/(activity)/ActivityType" />
        <View className="w-full h-36 items-center">
          <Image className="w-full h-full" resizeMode="contain" source={stone} />
        </View>
        <View className="px-6 mb-3">
          <Text className="text-white text-2xl font-extrabold">Where is it?</Text>
        ...
        <View className="flex-row flex-wrap justify-center px-4 gap-3">
          {OPTIONS.map((o) => (
            <TouchableOpacity key={o.id} activeOpacity={0.8} onPress={() => handleSelect(o.id)}
        ...
        {isInternational && (
          <View className="mt-6 px-6 gap-4">
            <LocationAutocomplete data={countries} label="Destination country" placeholder="Search country..."
              onSelect={(loc: any) => { setCountryIso(loc.isoCode); setCity(""); }} />
            {countryIso && (
              <LocationAutocomplete data={cities} label="Destination city" placeholder="Search city..."
                onSelect={(loc: any) => setCity(loc.name)} />
            )}
          </View>
        )}
      </KeyboardAwareScrollView>
      <View className="absolute bottom-0 left-0 right-0 px-6 pb-6 pt-4 items-center" style={{ backgroundColor: bg }}>
        <TouchableOpacity onPress={handleNext} disabled={!canConfirm}
