...
import { setCadre } from "@/redux/slices/activitySlices";
...
export default function Cadre() {
  ...
  const handleNext = () => {
    ...
    if (!item) return;
    dispatch(setCadre(item.label));
    router.push("/(screens)/(activity)/ActivitySummary");
  };

  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: bg }}>
      <HeaderQu linkPrv="/(screens)/(activity)/Moods" linkNext="/(screens)/(activity)/ActivitySummary" />
      <View className="w-full h-36 items-center">
        <Image className="w-full h-full" resizeMode="contain" source={stone} />
      </View>
      <View className="px-6 mb-3">
        <Text className="text-white text-2xl font-extrabold">What setting?</Text>
        <Text className="text-gray-400 text-sm mt-1">Choose your ideal environment</Text>
      </View>
