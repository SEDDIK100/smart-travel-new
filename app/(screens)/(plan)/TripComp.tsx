...
import { setTripComp } from "@/redux/slices/tripSlices";
...
export default function TripComp() {
  ...
  const handleNext = () => {
    ...
    if (!item) return;
    dispatch(setTripComp(item.label));
    router.push("/(screens)/(plan)/TripDuration");
  };

  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: bg }}>
      <HeaderQu linkPrv="/(screens)/(plan)/TripMood" linkNext="/(screens)/(plan)/TripDuration" />
      ...
      <View className="px-6 mb-3">
        <Text className="text-white text-2xl font-extrabold">Who s coming?</Text>
        <Text className="text-gray-400 text-sm mt-1">Who are you traveling with</Text>
      </View>
