...
import { setTripStyle } from "@/redux/slices/tripSlices";
...
export default function TripStyle() {
  ...
  const handleNext = () => {
    ...
    if (!item) return;
    dispatch(setTripStyle(item.label));
    router.push("/(screens)/(plan)/TripBudget");
  };

  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: bg }}>
      <HeaderQu linkPrv="/(screens)/(plan)/Interrests" linkNext="/(screens)/(plan)/TripBudget" />
      ...
      <View className="px-6 mb-3">
        <Text className="text-white text-2xl font-extrabold">Travel style?</Text>
        <Text className="text-gray-400 text-sm mt-1">What style suits you</Text>
      </View>
