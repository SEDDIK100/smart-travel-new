...
import { setDestinationType as setAction } from "@/redux/slices/tripSlices";
import { router } from "expo-router";
import HeaderQu from "@/components/HeaderQu";

const options = [
  { id: 1, label: "City", icon: "🏙️" },
  { id: 2, label: "Beach", icon: "🏖️" },
  { id: 3, label: "Mountains", icon: "⛰️" },
  { id: 4, label: "Nature", icon: "🌲" },
  { id: 5, label: "Surprise", icon: "🎲" },
];
...
const DestinationType = () => {
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#0d0d0d]">
      <HeaderQu linkPrv="/(screens)/(plan)/TripName" linkNext="/(screens)/(plan)/TripMood" />
      ...
      <View className="px-6 mb-4">
        <Text className="text-white text-2xl font-extrabold tracking-tight">Destination type ?</Text>
        <Text className="text-gray-400 text-md mt-1">What kind of place do you want to visit</Text>
      </View>
